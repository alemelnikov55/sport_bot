...
# RUN
# ===========================================
# async def build_running_results_payload(session: AsyncSession, distance_m: int) -> dict:
#     # 1. Формируем название дисциплины
#     sport_name = f"Бег {distance_m} м"
#
#     # 2. Получаем локальный sport_id
#     sport_id = await session.scalar(
#         select(Sport.sport_id).where(Sport.name.ilike(sport_name))
#     )
#     if not sport_id:
#         raise ValueError(f"❌ Вид спорта '{sport_name}' не найден в таблице sports")
#
#     # 3. Получаем внешний disciplineId
#     discipline_id = await session.scalar(
#         select(ExternalSportMapping.external_id).where(ExternalSportMapping.sport_id == sport_id)
#     )
#     if not discipline_id:
#         raise ValueError(f"❌ Нет external_id для '{sport_name}' в ExternalSportMapping")
#
#     # 4. Загружаем все результаты по дистанции
#     result_rows = await session.execute(
#         select(RunningResult).where(RunningResult.distance_m == distance_m)
#     )
...
#         "results": [
#             {
#                 "athleteId": res.participant_id,
#                 "result": int(float(res.result_time) * 1000)  # Перевод сек → мс
#             }
#             for res in results
#         ]
#     }
