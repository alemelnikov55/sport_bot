...
class UpdateTournamentStagesRequest(BaseModel):
    """
    UpdateTournamentStagesRequest
    """ # noqa: E501
    discipline_id: Optional[StrictStr] = Field(default=None, description="Идентификатор дисциплины", alias="disciplineId")
    tournament: Optional[Annotated[List[TournamentStagesInner], Field(min_length=0)]] = Field(default=None, description="Турнирная сетка")
    __properties: ClassVar[List[str]] = ["disciplineId", "tournament"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in tournament (list)
        _items = []
        if self.tournament:
            for _item in self.tournament:
                if _item:
                    _items.append(_item.to_dict())
            _dict['tournament'] = _items
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "tournament": [TournamentStagesInner.from_dict(_item) for _item in obj["tournament"]] if obj.get("tournament") is not None else None
        })
        return _obj
