...
class CreateResultRequest(BaseModel):
    ...
    CreateResultRequest
    """ # noqa: E501
    correlation_id: StrictStr = Field(description="Идентификатор пакета результатов. Можно любой рандомный уникальный, например GUID. Будет использоваться для предотвращения повторной обработки одного и того же запроса", alias="correlationId")
    discipline_id: StrictStr = Field(description="Идентификатор дисциплины", alias="disciplineId")
    competition_id: Optional[StrictStr] = Field(default=None, description="Идентификатор игры из турнирной сетки, если дисциплина isTournament=true", alias="competitionId")
    results: Annotated[List[Optional[Dict[str, Any]]], Field(min_length=1)] = Field(description="Если есть competitionId, то в результатах должно быть только две команды")
    __properties: ClassVar[List[str]] = ["correlationId", "disciplineId", "competitionId", "results"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "disciplineId": obj.get("disciplineId"),
            "competitionId": obj.get("competitionId"),
            "results": [OneOf.from_dict(_item) for _item in obj["results"]] if obj.get("results") is not None else None
        })
        return _obj
